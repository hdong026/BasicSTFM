"""Shared robust Stage-I (stable-law) objective: group-wise EMA risk, optional reweighting.

Used by StableResidualForecastingTaskV3 (DPM-v3 backbone) and StableResidualForecastingTaskV5
(DPM-SR / SRDSTFM). Stage II/III always delegate to StableResidualForecastingTask.step.
"""
...
class RobustStage1TaskMixin:
    """Adds optional robust stable-law Stage-I; keep super().step for all other phases."""
    ...
    def _per_group_m_g(self, l_g: Tensor, g: str) -> Tensor:
    ...
    def step(self, model: torch.nn.Module, batch: Dict[str, Any], losses, device: torch.device):
        ...
        if not isinstance(outputs, dict):
            raise TypeError("RobustStage1TaskMixin expects model outputs to be a dict")
