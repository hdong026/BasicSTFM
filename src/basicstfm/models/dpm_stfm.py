"""SRD-STFM: Stable-Residual Diffusion spatio-temporal foundation backbone."""
...
@MODELS.register("MADSTFMBackbone")
@MODELS.register("DPMSTFMBackbone")
@MODELS.register("SRDSTFMBackbone")
class SRDSTFMBackbone(nn.Module):
    """Foundation backbone that first learns stable trunk, then residual diffusion."""
    ...
    def load_backbone_weights(self, path: str, strict: bool = False) -> tuple[list[str], list[str]]:
        """Load stable + diffusion backbone while skipping calibration/fusion heads."""
    ...
    def load_stable_trunk_weights(self, path: str, strict: bool = False) -> tuple[list[str], list[str]]:
        """Load only stable trunk weights for staged transfer."""
