...
class ForecastWithAuxEmbeddingPenalty(Task):
    ...
    def step(self, model: torch.nn.Module, batch: Dict[str, Any], losses, device: torch.device):
        ...
        x = self.transform(batch["x"], batch=batch)
        outputs = model(x, graph=batch.get("graph"), mode="both")
        forecast = self.inverse_transform(outputs["forecast"], batch=batch)
        mask = self.merge_masks(batch.get("y_mask"))
        target, mask = self.align_prediction_target(forecast, batch["y"], mask)
        loss_out = losses(forecast, target, mask=mask)
